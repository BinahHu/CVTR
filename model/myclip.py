...
class Transformer(nn.Module):
    ...
    def rebuild_mask(self, mask):
        for i in range(len(self.resblocks)):
            self.resblocks[i].attn_mask = mask
            self.resblocks[i].attn_mask.requires_grad = False
...
class VisionTransformer(nn.Module):
    ...
    def forward_with_prompts(self, x, prompts):
        ...
        x = torch.cat([prompts.to(x.dtype) + torch.zeros(x.shape[0], prompts.shape[1], x.shape[-1], dtype=x.dtype,
        ...
        x = self.ln_post(x[:, :prompts.shape[1] + 1, :])
    ...
    def add_prompt(self, vision_prompt_num=0, text_prompt_num=0, freeze_old_prompts=True, prompt_init="random", seq=False):
        ...
        if text_prompt_num > 0:
            ...
            if self.text_prompt_per_task != 0:
                ...
                self.text_prompt_per_task = text_prompt_num
            self.text_prompt_list.append(text_prompt)
            self.text_prompt_num += text_prompt_num
            mask = self.build_attention_mask().to(self.positional_embedding.device)
            self.transformer.rebuild_mask(mask)

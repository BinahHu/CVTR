    ...
    def video_to_tensor(self, video_file, preprocess, sample_fp=0, start_time=None, end_time=None):
        ...
        for retry in range(10):
            ...
            if fps > 0:
                ...
                print(f"FPS = 0 Detected! Retry for the {retry+1} times")
                cap.release()

        if fps == 0:
            print(f"Finally, FPS = 0 Detected!")
        ...
        assert len(inds) >= sample_fp, f"sample fp is {sample_fp}, len ind is {len(inds)}, fps is {fps}"
        inds = inds[:sample_fp]

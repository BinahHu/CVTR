...
def main(args,config,industry_id_label, all_id_info, device): 
    ...
    model = clip_pretrain(config=config, image_size=config['image_size'], vit=config['vit'],
                          ...
                          chinese_clip=True, clip=False, zero_shot=True)
   ...
   #####task_list########
    ...
    for iteration , task_i in enumerate(config['task']):
        ...
        if iteration==0:
            ...
            test_list.append(task_i)
            if config.get('mode', 'seq') == 'bound':
                train_list.append(task_i)
            else:
                train_list=[task_i]

        if iteration < len(config['task']) -1 :
            print(f"Bypass previous tasks since it is zero shot setting")
            continue

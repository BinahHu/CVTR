...
def main(args,config,industry_id_label, all_id_info, device): 
    ...
    model = clip_pretrain(config=config, image_size=config['image_size'], vit=config['vit'],
                          ...
                          prompts_start_layer=6, prompts_end_layer=-1,
   ...
   #####task_list########
    ...
    for iteration , task_i in enumerate(config['task']):
        ...
        pretrain_train_dataset = create_dataset('product_train', config, industry_id_label = industry_id_label, all_id_info=all_id_info, task_i_list=train_list[:3])
        pretrain_test_dataset = create_dataset('product_test', config, task_i_list=test_list[:3])
        pretrain_datasets = [pretrain_train_dataset,pretrain_test_dataset]
        print('number of pretraining training samples: %d'%len(pretrain_datasets[0]))
        print('number of pretraining testing samples: %d'%len(pretrain_datasets[1]))
        if args.distributed:
            num_tasks = utils.get_world_size()
            global_rank = utils.get_rank()            
            pretrain_samplers = create_sampler([pretrain_train_dataset], [True], num_tasks, global_rank)+ [None]
        else:
            pretrain_samplers = [None,None]

        pretrain_data_loader, pretrain_test_loader = create_loader(pretrain_datasets,pretrain_samplers,batch_size=[config['batch_size_train'], config['batch_size_test']], num_workers=[8,8], is_trains=[True,False], collate_fns=[None,None])


        print("Creating dataset")
        train_dataset = create_dataset('product_train', config, industry_id_label=industry_id_label,
                                       all_id_info=all_id_info, task_i_list=train_list[3:])
        test_dataset = create_dataset('product_test', config, task_i_list=test_list[3:])
        ...
        query_dataset, galley_dataset = create_dataset('product_query', config,
                                                       task_i_list=test_list[3:]), create_dataset(
            'product_gallery', config, task_i_list=test_list[3:])
        query_loader, gallery_loader = create_loader([query_dataset, galley_dataset], [None, None],
                                                     batch_size=[128, 128], num_workers=[8, 8],
        ...
        score_test_i2t, score_test_t2i = evaluation(model_without_ddp, pretrain_test_loader, device, args, config)
        if utils.is_main_process():
            test_result = itm_eval(score_test_i2t, score_test_t2i, pretrain_test_loader.dataset.txt2img,
                                   pretrain_test_loader.dataset.img2txt)
            print("Show zero shot res on pretraining dataset")
            print(test_result)
        ...
        if utils.is_main_process():
            test_result = itm_eval(score_test_i2t, score_test_t2i,
                                   ...
                                   test_loader.dataset.img2txt)
            print("Show zero shot res on actual dataset")
            print(test_result)

        for epoch in range(0, config['max_epoch']):
            train_stats = update_train(model, pretrain_data_loader, optimizer, epoch, device, config, iteration, lr_schedule,
                                       model_without_ddp, pretrain_test_loader, query_loader, gallery_loader, mode='stage_one')
            score_test_i2t, score_test_t2i = evaluation(model_without_ddp, pretrain_test_loader, device, args, config)
            if utils.is_main_process():
                test_result = itm_eval(score_test_i2t, score_test_t2i, pretrain_test_loader.dataset.txt2img, pretrain_test_loader.dataset.img2txt)
                print(test_result)
        ...
        if utils.is_main_process():
            test_result = itm_eval(score_test_i2t, score_test_t2i,
                                   test_loader.dataset.txt2img,
                                   test_loader.dataset.img2txt)
            print("After stage one, pretrained model res on actual dataset")
            print(test_result)

        print("pretrain stage end, enter stage two")
        model.module.to_stage_two_pretrained()
        model_params = {n: p for n, p in model.module.named_parameters() if p.requires_grad}
        # for name, p in model.module.named_parameters():
        #     if p.requires_grad:
        #         print(name)
        ...
        for epoch in range(0, config['max_epoch']):
            train_stats = update_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule,
                                       model_without_ddp, test_loader, query_loader, gallery_loader)
            score_test_i2t, score_test_t2i = evaluation(model_without_ddp, test_loader, device, args, config)
            if utils.is_main_process():
